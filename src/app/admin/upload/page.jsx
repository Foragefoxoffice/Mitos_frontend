...
// Dynamically import react-select and papaparse to avoid SSR issues
...
const CSVImporter = () => {
  ...
  // Update chapterId, subjectId, and portionId when topicId changes
  useEffect(() => {
    if (topicId) {
    ...
    <div className="py-10">
      <form onSubmit={handleSubmit}>
        ...
        <div className="p-20 my-8 border-dashed border-2 grid gap-3 place-items-center">
          <label className="text-2xl font-bold text-[#B9B9B9]" htmlFor="file">
            Drag & Drop to Upload
          </label>
          <p className="text-2xl font-bold text-[#B9B9B9]">or</p>
          <label className="file_upload" htmlFor="file">
            <FaPlus size={40} className="file_icon" /> Select Files to Upload
          </label>
          <input
            type="file"
            name="file"
