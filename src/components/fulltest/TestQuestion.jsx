...
export const TestQuestion = ({
  ...
  (question) => {
    return question.options.map((option, index) => {
      ...
      let buttonClass = "flex items-center gap-2 w-full text-left px-4 py-3 rounded-lg border transition-all duration-150 ";
      ...
      if (onShowAnswers) {
        if (isCorrect) {
          ...
          buttonClass += "bg-[#FAF5FF] text-[#282C35] border border-[#C5B5CE] opacity-60";
        }
      } else {
        if (isSelected) {
      ...
      return (
        <button
          key={index}
          onClick={() => !onShowAnswers && handleAnswer(question.id, currentOptionLabel)}
